...
public:
  ...
  PlanningVisualization(ros::NodeHandle& nh, const std::string& frame);

  //delete all Marks
  void remove_all_markers(std::vector<int>& marker_ids);
  ...
  void displaySphereList(const vector<Eigen::Vector2d>& list, double resolution,
                         const Eigen::Vector4d& color, int pub_id = 0);                      
  ...
  void displayCylinderList(const Eigen::Vector3d pos,int id, int pub_id = 0);
  ...
  void executeTraj(NonUniformBspline& bspline);
